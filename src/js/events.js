...
const event_template = (event, isPastEvent) => {
  ...
  div.innerHTML = `
    ...
    <div class="event__info">
      ...
      <div class="event__info__child event__duration">${event.hourss}</div>
      <div class="event__info__child event__description">Address : ${isDescriptionOverflow ? description_short + '...' : description_long}${isDescriptionOverflow ? expand_button_html : ''}</div>
      ${((event.additional_buttons && event.additional_buttons.length) || (event.vod && event.vod.length)) || !isPastEvent ?
        `<div class="event__info__child event__buttons"><p>${event.vod}</p>
          ...
          ${
            event.vod && `
              <a class="event__button-container vod-button-container" href="tel:${event.vod}" target="_blank">
                <button class="event__button vod-button"><i class="fa fa-phone-alt"></i>Call</button>
              </a> 
            `
          }
          ${
...
if(upcoming_events_data.length > 0) {
  ...
  upcomingEventsHTMLElement.innerHTML = no_event_template(' ');
}
